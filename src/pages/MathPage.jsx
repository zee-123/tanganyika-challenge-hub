      ...
      <div className="max-w-3xl mx-auto px-4 relative z-10">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          ...
          <div className="flex justify-center gap-2 mt-3 flex-wrap">
            {['🔢 Maths Cadet', '⚔️ Maths Warrior', '🏆 Maths Legend'].map(b => (
          ...
          <div className="mt-3 px-4 py-2 rounded-xl inline-block text-sm text-pink-200"
            ...
            ⚔️ Correct answer = attack monster! Wrong answer = monster attacks back!
          </div>
        </motion.div>
        <QuizEngine subject="math" pool={mathPool} gradient={gradient} icon="⚔️" monsterMode={true} />
      </div>
    </div>
  );
}
