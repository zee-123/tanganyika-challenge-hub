...
export default function OlympiadPage() {
  ...
  const [qs, setQs] = useState(() => getRandomOlympiadQuestions(resolvedKey, QUESTIONS_PER_ROUND));
  ...
  const handleAnswer = (opt) => {
    ...
    if (correct) {
      ...
      addPoints('olympiad', q.points);
      confetti({
        particleCount: 80, spread: 75, origin: { y: 0.6 },
        colors: ['#667eea', '#764ba2', '#fbbf24', '#f472b6'],
      });
    }
    advance(correct);
  };
  ...
  const restart = (yr = selectedYear) => {
    setQs(getRandomOlympiadQuestions(yr, QUESTIONS_PER_ROUND));
  ...
  const timerColor = timeLeft > 30 ? '#34D399' : timeLeft > 15 ? '#FBBF24' : '#F87171';
  ...
  if (finished) {
    ...
    return (
      <div className="min-h-screen pt-28 pb-10 relative"
        style={{ background: 'linear-gradient(135deg, #0d0221 0%, #1a0533 50%, #0a011a 100%)' }}>
        <StarField />
        <div className="max-w-lg mx-auto px-4 relative z-10 pt-4">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="mb-5 text-left">
              <p className="text-white/60 text-xs uppercase font-bold tracking-wider mb-2">Subject Breakdown</p>
                  ...
                  <div key={cat} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 h-2 rounded-full bg-white/10">
                      <div className="h-2 rounded-full transition-all"
                        style={{ width: `${Math.round((c / t) * 100)}%`, background: CAT_COLORS[cat] || '#a78bfa' }} />
                    </div>
      ...
      <div className="max-w-2xl mx-auto px-4 relative z-10">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-purple-300 text-sm font-semibold">Q {currentQ + 1} of {qs.length}</span>
            <div className="flex gap-0.5">
              {qs.map((_, i) => (
                <div key={i} className="w-1.5 h-1.5 rounded-full transition-all"
                  style={{ background: i < currentQ ? '#764ba2' : i === currentQ ? 'white' : 'rgba(255,255,255,0.2)' }} />
              ))}
            </div>
          </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={currentQ} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            <div className="glass rounded-2xl p-6 mb-5"
              style={{ border: `1px solid ${(CAT_COLORS[q.cat] || '#a78bfa')}40` }}>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-bold px-3 py-1 rounded-full"
                  style={{ background: `${CAT_COLORS[q.cat] || '#a78bfa'}20`, color: CAT_COLORS[q.cat] || '#a78bfa', border: `1px solid ${CAT_COLORS[q.cat] || '#a78bfa'}40` }}>
                  {q.cat}
                </span>
                <span className="text-yellow-400 text-xs font-bold ml-auto">+{q.points} pts</span>
                <span className="text-purple-400 text-xs">Cambridge Level</span>
              </div>
